...
public class PagamentoDAOImpl implements PagamentoDAO{
    ...
    private final String FIND_PAGAMENTO_ID = "select p.id as pid, p.ordineArrivo as pordineArrivo, p.data as pdata, p.quota as pquota, p.Ragazzo_id as pragid, re.id as reid, re.mail as remail, re.password as repassword, re.nome as renome, re.cognome as recognome, re.telefono as retelefono, re.localita as relocalita, re.via as revia, re.civico as recivico, re.tipoUt as retipoUt"
            + " from Pagamento p "
    ...
    private final String FIND_ALL_PAGAMENTO = "select p.id as pid, p.ordineArrivo as pordineArrivo, p.data as pdata, p.quota as pquota, p.Ragazzo_id as pragid, re.id as reid, re.mail as remail, re.password as repassword, re.nome as renome, re.cognome as recognome, re.telefono as retelefono, re.localita as relocalita, re.via as revia, re.civico as recivico, re.tipoUt as retipoUt"
            + " from Pagamento p "
            + " join Registrato re on (p.Registrato_id = re.id) ";                                                                    
    private final String FIND_PAGAMENTO_REGISTRATO_ID = "select p.ordineArrivo as pordineArrivo, p.id as pid, p.data as pdata, p.quota as pquota, p.Ragazzo_id as pragid, re.id as reid, re.mail as remail, re.password as repassword, re.nome as renome, re.cognome as recognome, re.telefono as retelefono, re.localita as relocalita, re.via as revia, re.civico as recivico, re.tipoUt as retipoUt"
            + " from Pagamento p "
            + " join Registrato re on (p.Registrato_id = re.id) "                                                                       
            + " where p.Registrato_id = ?;";
    private final String FIND_PAGAMENTO_RAGAZZO_ID = "select p.ordineArrivo as pordineArrivo, p.id as pid, p.data as pdata, p.quota as pquota, p.Ragazzo_id as pragid, re.id as reid, re.mail as remail, re.password as repassword, re.nome as renome, re.cognome as recognome, re.telefono as retelefono, re.localita as relocalita, re.via as revia, re.civico as recivico, re.tipoUt as retipoUt"
            + " from Pagamento p "
            + " join Registrato re on (p.Registrato_id = re.id) "                                                                       
            + " where p.Ragazzo_id = ?;";
    ...
    @Override
    public void insert(int ordineArrivo, java.util.Date data, float quota, int ragId, int regId) throws SQLException {
        Connection con = DAOMan.getConnection();
        PreparedStatement pst = con.prepareStatement(INSERT_PAGAMENTO);
